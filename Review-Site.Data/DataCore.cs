...
namespace Review_Site.Data
{
    public static class DataCore
    {
        ...
        public static void SeedBaseData()
        {
            ...
            foreach (Permission permission in GetPermissionCollection("Admin", db))
            {
            ...
            if (!db.Users.Any(x => x.ID == new Guid("4dd27ddf-995c-43cf-9867-43e4f9bee080")))
            {
                db.Users.AddOrUpdate(new User
                {
                    ...
                    Roles = new Role[]{
                    db.Roles.Single(x => x.Name == "Global Administrator")
                },
                    Created = DateTime.Now,
                    LastModified = DateTime.Now
                });
        ...
        private static System.Collections.Generic.IList<Permission> GetPermissionCollection(string partialIdent, DataContext db)
        {
            List<Permission> permissions = db.Permissions.Get().Where(x => x.Identifier.StartsWith(partialIdent)).ToList();
            return permissions;
        }

        private static Permission GetPermission(string identifier, DataContext db)
        {
